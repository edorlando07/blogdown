...
Each playlist was previously converted to .xlsx files and placed into the appropriate genre's folder within the R project.  The three genre folders are Country, Hip-Hop, and Rock.
...
A function was created that can list the .xlsx files in each genre folder. The function utilizes the **base::list.files()** function that lists each file under a particular folder.
...
Since the entire path and file name are all listed in the **filename** column, the **separate()** function was used to split out three (3) folders, Genre, and the Playlist into five (5) separate columns.
...
The final tibble is complete and tidy.
