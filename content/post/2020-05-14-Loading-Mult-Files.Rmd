...
### Create Genre List
Next, the **genre_list()** is generated which includes all three (3) genres.  This list will allow us to map the **list_files_in_folder()** function to multiple genre folders later on.
```{r}
...
The **playlist_list$value** was mapped into the function below which loaded each .xlsx file.  The code below also added the filename and entire path into a column called **filename**.
...
The entire path and filename are all listed in the **filename** column in the **song_tbl**.  Using the **separate()** function we can split out three (3) folders, Genre, and the Playlist into five (5) separate columns.
