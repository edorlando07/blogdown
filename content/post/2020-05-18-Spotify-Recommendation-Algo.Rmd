...
This tutorial shows you some of the methods used to create these types of algorithms.  In the example presented, a user will be able to select an musical artist and a song they enjoy.  The algorithm will then populate 10 new songs that the user might also like.  For more info about Spotify and the products they offer, click **[here](<https://www.spotify.com/us/about-us/contact/>)**.
...
We can also see which songs appear the most across multiple playlists.  It is common for many companies to include "most popular" items as part of their recommendations to customers.  We will compare a "most popular" recommendation algorithm to other algorithms a little later. 
...
In order for the Recommendation algorithm to run more efficiently, we can reduce the number of songs that do not appear at least 3 times.  We also need to convert the condensed playlist tibble into a "wide" format. 
...
The data is now in the proper "wide" format using 1s and 0s to identify if a song is included in a particular playlist.  Below is a sample of the first 10 columns and 10 rows of the widely formatted tibble.
...
The recommendation algorithms then need the data converted into a matrix format.  The code below performs that transformation.
...
One popular method used to identify freqent item sets is called **[Association Rules (arules)](<https://cran.r-project.org/web/packages/arules/arules.pdf>)**.  In order to identify a solid arules model, we will set up and compare 4 different models using 4 different paramters in each.   

In order to do so, we first need to set up a recipe for the recommendation algorithm using the evaluationScheme function.  For more information related to this package, please click **[here](<https://www.rdocumentation.org/packages/recommenderlab/versions/0.2-5/topics/evaluationScheme>)**.
...
Next, we will now set up 4 various Association Rules settings so that each algorithm can be compared to find the most appropriate support and confidence levels.  For more information related to the **recommenderlab::evaluate()** please click [here](<https://cran.r-project.org/web/packages/recommenderlab/vignettes/recommenderlab.pdf>).
```{r}
...
In the code below, we pull out the True Positive Rate (TPR) as well as the False Positive Rate (FPR) in order to compare and contrast the various parameters in the four (4) Association Rules algorithms.
```{r}
...
The **arules_model_01** performs the best since we get most results/recommendations back.  In other words, arules_model_01 produced 10 song recommendations, and the other models did not.  The other algorithms fall short of that goal either because the support or confidence levels were set too high.

Going forward, we will only use the arules_model_01 since it performed the best.
...
Just as we did previously, we will set up the algorithm list.
...
The best performing model based on the true positive rate (TPR) and false-positive rate (FPR) was the **[User-Based Collaborative Filtering (UBCF)](<https://cran.r-project.org/web/packages/recommenderlab/vignettes/recommenderlab.pdf>)** algorithm.  We will use this model to produce the recommended playlist below.
...
### Set Up Favorite Song
We can make playlist recommendations using the model_ucbf and by passing through 1 song.  For this example, we will plug in one my favorite songs written by the Rolling Stones called "Paint It Black."
...
Below we are passing the artist/track name into the model to produce a recommended playlist of 10 songs.  Some songs recommended include Jerry Lee Lewis - Great Balls of Fire as well as Foghat - Slow Ride.  For the full list of recommendations, please see the list below.  
```{r}
...
### Next steps
In the next few weeks, I plan to create a Shiny app which will allow you to enter your favorite song and receive a recommended playlist.  For questions or comments about this article, please send me a message on [linkedin](<https://www.linkedin.com/in/edorlando07/>).
