...
### Analyzing the Data: Which Genres Include the Most Playlists?
...
The data was transformed using 1s and 0s to identify if a song was included in a particular playlist.  Below is a sample of the first 10 columns and 10 rows of the widely formatted tibble.
...
One popular method used to identify frequent item sets is called **[Association Rules (arules)](<https://cran.r-project.org/web/packages/arules/arules.pdf>)**.  In order to identify an appropriate arules model, we set up and compared 4 different arules models.   
...
Although models 2-4 showed a more promising ROC curve, the **arules_model_01** was selected since it produced the most results/recommendations back.  In other words, arules_model_01 produced 10 song recommendations, while the other models did not.  The other algorithms fall short of the 10 song goal either because the support or confidence levels were set too high.
...
We made the playlist recommendations using the **model_ucbf** by passing through 1 song.  As mentioned earlier, we plugged in one my favorite songs by the **Rolling Stones** called *Gimme Shelter*.
